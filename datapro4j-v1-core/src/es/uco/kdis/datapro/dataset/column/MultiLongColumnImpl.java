...
/**
 * This class represents the implementation of a multivalue column of long values. It 
 ...
 *
 **/
public class MultiLongColumnImpl extends MultiColumnImpl {

	/**
	 * Default constructor.
	 @since 0.1
	 **/
	public MultiLongColumnImpl() {
	}
	...
	public int addValue(Object oValue) {
		try {
			if (oValue == null) {
				...
				MultiLongValue oMultiValue = new MultiLongValue();
				oMultiValue.addValue((Long)oValue);
				this.rgoValues.add(oMultiValue);
				return 1;
			}
			else if (oValue instanceof MultiLongValue || oValue instanceof InvalidValue) {
	...
	public int addElementToMultivalue(Object oValue, int iIndex) {
		try {
			if (oValue instanceof Long && this.rgoValues.get(iIndex) instanceof MultiLongValue)
				return ((MultiLongValue)this.rgoValues.get(iIndex)).addValue((Long) oValue);
			else
				return 0;
		} catch (Exception e) {
			return 0;
	...
	public int addElementToMultivalue(Object oValue, int iIndex, int iInternalIndex) {
		try {
			if (oValue instanceof Long && this.rgoValues.get(iIndex) instanceof MultiLongValue)
				return ((MultiLongValue)this.rgoValues.get(iIndex)).addValue((Long) oValue, iInternalIndex);
			else
				return 0;
		} catch (Exception e) {
			return 0;
	...
	public int addAllElementsToMultivalue(List<Long> rgoValues, int iIndex) {
		try {
			if (this.rgoValues.get(iIndex) instanceof MultiLongValue)
				return ((MultiLongValue)this.rgoValues.get(iIndex)).addAllValues(rgoValues);
			else
				return 0;
		} catch (Exception e) {
			return 0;
	...
	public int addElementsToMultivalue(List<Long> rgoValues, int iIndex, int iFromIndex, int iToIndex) {
		try {
			if (this.rgoValues.get(iIndex) instanceof MultiLongValue)
				return ((MultiLongValue)this.rgoValues.get(iIndex)).addValues(rgoValues, iFromIndex, iToIndex);
			else
				return 0;
		} catch (Exception e) {
			return 0;
	...
	public ArrayList<Long> getElementsOfMultivalue(int iIndex, int iFromIndex, int iToIndex) {
		try{
			if(this.rgoValues.get(iIndex) instanceof MultiLongValue)
				return ((MultiLongValue)this.rgoValues.get(iIndex)).getValues(iFromIndex,iToIndex);
			else
				return null;
		}catch(Exception e){
			return null;
	...
	public int setElementOfMultivalue(Object oValue, int iIndex, int iInternalIndex) {
		try {
			if (oValue instanceof Long && this.rgoValues.get(iIndex) instanceof MultiLongValue)
				return ((MultiLongValue)this.rgoValues.get(iIndex)).setValue((Long) oValue, iInternalIndex);
			else
				return 0;
		} catch (Exception e) {
			return 0;
	...
	public MultiDoubleColumn toMultiDouble(String sName){
		...
		for (int i = 0; i < iColSize; i++){
			...
			else if (oValue instanceof MultiLongValue) {
				oMultiValue = ((MultiLongValue)oValue).toMultiDouble();
	 ...
	 * @return MultiIntegerColumn where each value is converted to an integer number.
	 @since 0.1
	 **/
	public MultiIntegerColumn toMultiInteger(String sName){
		...
		for(int i = 0; i < iColSize; i++){
			...
			if (oValue == null){
				...
				oMultiValue = ((MultiLongValue)oValue).toMultiInteger();
				oResultColumn.addValue(oMultiValue);
			}
		}
		return oResultColumn;
